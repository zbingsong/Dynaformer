...
class GraphormerPYGDataset(Dataset):
    ...
    # torch_geometric Dataset expects a `get` method (not __getitem__)
    def get(self, idx: int):
        # delegate to your cached __getitem__ so you keep the existing behavior
        return self.__getitem__(idx)

    # torch_geometric Dataset expects a `len` method (not __len__)
    def len(self):
        return self.num_data
